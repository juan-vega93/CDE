...
export async function deleteBcfTopic(
  ...
  if (!normalizedProjectCode) {
    throw new Error("projectCode es obligatorio para eliminar topics BCF");
  }

  const response = await bffFetch(
    `/api/bcf/topics/${encodeURIComponent(topicId)}?projectCode=${encodeURIComponent(
      normalizedProjectCode
    )}`,
    {
      method: "DELETE"
  ...
  if (!response.ok) {
    throw new Error("No se pudo eliminar el topic BCF");
  }
}
